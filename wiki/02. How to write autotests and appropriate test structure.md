...
6. `checkDuring` <br>
It's a method that receives a lambda and invokes it in the same manner as `CheckDuringProviderImpl` does.
...
The example is [here](../sample/src/androidTest/java/com/kaspersky/kaspressample/flaky_tests/CheckDuringTest.kt).
7. ```compose``` <br>
