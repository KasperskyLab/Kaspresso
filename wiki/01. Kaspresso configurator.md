...
Developer also can extends parametrized tests functionality by providing ```BaseMainSectionEnrichers``` in ```BaseTestCase``` or ```BaseTestCaseRule```.
The main idea of enrichers - allow adding additional test case's steps before and after the main section's ```run``` block.

All you need to do is:

1. Define your enricher implementation inheriting ```BaseMainSectionEnricher``` class;

```kotlin
class LoggingMainSectionEnricher : BaseMainSectionEnricher<TestCaseData>() {
...
2. Inside ```enrichMainSection``` method add your before/after actions;
...
In ```beforeMainSectionRun``` and ```afterMainSectionRun``` blocks you have full access to ```TestContext<TestCaseData``` properties and methods,
so you can use logger, add new test case steps and so on. Also, this blocks have ```TestInfo``` parameter.

3. Add your enricher into your ```BaseTestCase``` implementation.
