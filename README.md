...
Чтобы все запустить:
##### **На стороне хоста**
Необходимо запустить клиент до выполнения тестов. Именно этот клиент будет исполнять adb-команды. Он представляет из себя jar-файл  **Desktop.jar**
Перед стартом тестов необходимо стартовать его, выполнив команду
```
java -jar desktop.jar
```
Данный джарник в уже собранном виде лежит в **./artifacts**
##### **На стороне девайса**
...
Некоторому API Kaspresso нужно наличие особых разрешений.
Для сохранения скриншотов шагов необходимо разрешение ```WRITE_EXTERNAL_STORAGE```.
Об остальных необходимых разрешениях написано в документации к методам.
