        ...
        when(instrumentalUsage) {
            is InstrumentalUsage.ComponentLocation ->
                """
                    ...
                    ${instrumentalUsage.componentName} is not adapted (or partially adapted only) to be executed like a Unit test (with Robolectric support on the JVM environment) because ${instrumentalUsage.componentName} calls $instrumentalProperty inside its implementation.
                    ...
                    2. If the reason of the crash is an Interceptor that uses ${instrumentalUsage.componentName} then there is recommendation to remove this Interceptor in case of the test executing on the JVM environment.
            ...
            is InstrumentalUsage.InterceptorLocation ->
                """
                    ...
                    ${instrumentalUsage.interceptorName} has calls of $instrumentalProperty inside its implementation. It means that ${instrumentalUsage.interceptorName} is not adapted to be executed in a Unit test (with Robolectric support on the JVM environment).
                    We suggest next options:
