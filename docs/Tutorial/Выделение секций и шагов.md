...
// image log_step_2_failed

Судя по логам step2 действительно завершился неудачно. Был проверен статус заголовка, текст не совпал, программа осуществила еще несколько попыток проверить, что текст на заголовке содержит текст `disabled`, но все эти попытки не увенчались успехом и шаг завершился с ошибкой. Почему в этом случае тесты у нас зеленые?
...
```
Запускаем тест еще раз. Теперь он завершается с ошибкой и мы имеем понятные логи, где сразу видно, на каком шаге произошла ошибка. После step2 в логах больше ничего нет.
...
Внутри каждого step-а мы указываем действия, которые требуются на этом шаге. Тоже самое, что мы делали раньше. Тогда код теста будет выглядеть следующим образом:
...
В Kaspresso есть возможность добавить блоки `before` и `after`. Код внутри блока `before` будет выполняться перед каждым тестом - здесь мы можем установить настройки по умолчанию. Код внутри блока `after` будет выполнен после каждого теста. Во время выполнения теста состояние телефона может меняться: мы можем выключить интернет, сменить ориентацию, но после теста нужно вернуть исходное состояние. Делать это мы будем внутри блока `after`.
