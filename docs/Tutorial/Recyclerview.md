...
Мы уже говорили ранее о том, что Page Object нам понадобится для каждого элемента списка, поэтому необходимо создать соответствующий класс, и экземпляр этого класса мы будем передавать в `itemTypeBuilder`.
...
В качестве параметра конструктора `KRecyclerViewItem` необходимо передать объект `matcher`. Зачем он нужен? Дело в том, что искать элементы мы будем по их id, но у всех элементов списка идентификаторы одинаковые, поэтому при поиске на всем экране их будет найдено несколько экземпляров, станет непонятно, какой конкретно элемент проверять, и тест завершится неудачно. 

Вместо этого поиск будет осуществляться отдельно для каждой заметки, и для каждой из них будет создан свой экземпляр Page Object. Во время тестирования для каждого объекта будет передан свой `matcher`, в котором мы найдем необходимые View-элементы.
...
Поэтому в случаях, когда какое-то действие выполняется с анимацией и для его завершения требуется время, можно вызвать метод `device.uiDevice.waitForIdle`. Этот метод остановит выполнения теста до тех пор, пока экран не перейдет в состояние idle (бездействующее, простоивающее) – когда не происходит никаких действий и не выполняются анимации.
